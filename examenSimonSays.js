...
async function comenzarJuego(nombre, rl) {
  ...
  let secuencia = generarSecuencia(4);
  ...
  let valida = false
  //let longitudActual = await leer(`${nombre}, digame el nivel de dificultad (Facil: f, Medio: n, Difícil: d):`);

  let longitudActual;
  while (!valida){
    longitudActual = await leer(`${nombre}, digame el nivel de dificultad (Facil: f, Medio: n, Difícil: d):`);
    if (longitudActual == "f"){
      longitudActual = 2
      valida = true
  ...
  while (!juegoTerminado && longitudActual <= MAX_COLORES_SEQ) {
    ...
    if (!juegoTerminado) {
      ...
      console.log("Puntuación final : "  + puntuacion + " puntos")
      longitudActual++;
    } else {
